...
1. 实现思路，v5中的common代码结构进行了保留，因为这一部分代码是比较好理解的，整体代码看起来是比较简单的，主要是整体网络结构的搭建，通过解析yaml文件对于一些开发人员来说是不是很友好的。
...
5. 四种尺寸的配置文件的写法，放在了config字典中，这是网络模型的配置参数，没有将其他的参数放到配置文件中，可以将类别也放到配置文件中。在上面的网络代码中宽度参数就是变量`e`然后传入到每个网络中去。
